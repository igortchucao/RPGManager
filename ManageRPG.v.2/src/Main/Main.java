...
public class Main extends Canvas implements KeyListener, Runnable, MouseListener {
	...
	private Menu menu;
	public funcoesAuxiliares fA = new funcoesAuxiliares();
	...
	public void mouseClicked(MouseEvent e) {
		// SE CLICKA PARA CRIAR UM PERSONAGEM
		if (menu.newIsCliked) {
			entidades.add(new Vertice(new Personagem(null, null, null), e.getX(), e.getY(), 100, 50));
			menu.newIsCliked = false;

			// SE CLICAR PARA CRIAR UMA NOVA RELAÇÃO
		} else if (menu.relacIsCliked == 1) {
			for (int i = 0; i < entidades.size(); i++) {
				Vertice v = entidades.get(i);
				if (v.clicked(e.getX(), e.getY())) {
					// ELE FOI O SELECIONADO PARA RELAÇÃO
					v.addRelacion = true;
					menu.relacIsCliked = 2;
				}
			}
			if (menu.relacIsCliked == 1)
				menu.relacIsCliked = 0;
			// APLIC. ESPERANDO QUAL É O ALVO
		} else if (menu.relacIsCliked == 2) {
			for (int i = 0; i < entidades.size(); i++) {
				Vertice v = entidades.get(i);
				if (v.addRelacion) {
					for (int j = 0; j < entidades.size(); j++) {
						Vertice f = entidades.get(j);
						if (f.clicked(e.getX(), e.getY())) {
							// PROCURO QUEM FOI O SELECIONADO E ADICIONO O ALVO
							v.personagem.cicloDeRelacionamento.add(
									new Relacionamento(0, f, fA.mediaModulo(v.Px + f.Px), fA.mediaModulo(v.Py + f.Py)));
							menu.relacIsCliked = 0;
						}
					}
					v.addRelacion = false;
			...
			if (menu.relacIsCliked == 2) {
				menu.relacIsCliked = 0;

			}
			// SE NAO TIVER NADA CLICADO, ESPERA CLICAR
		} else {
			// BOTAO "NOVO"
			menu.newClicked(e.getX(), e.getY());
			// BOTAO "RELACAO"
			menu.relacClicked(e.getX(), e.getY());
			// BOTAO "RELACAO"
			menu.editRClicked(e.getX(), e.getY());
			// CAIXA DE DETALE
			for (int i = 0; i < entidades.size(); i++) {
				Vertice v = entidades.get(i);
				v.personClicked(e.getX(), e.getY());
			}
		}
	}
